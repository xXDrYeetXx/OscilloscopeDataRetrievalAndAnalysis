...
def configure_scope(scope) -> None:
    ...
    # Configure binary waveform transfer once.
    scope.write(
...
def read_channel_waveform_binary(scope):
    ...
    The calibrated voltage is reconstructed as:
    ...
    y_increment = query_float(
        scope,
        ":WAVeform:YINCrement?",
    )

    y_origin = query_float(
        scope,
        ":WAVeform:YORigin?",
    )

    y_reference = query_float(
        scope,
        ":WAVeform:YREFerence?",
    )
...
class SingleBinAnalyzer:
    ...
    def __init__(
        ...
        self.window_sum = float(
            np.sum(self.window)
        )

        self.window_square_sum = float(
            np.sum(self.window ** 2)
        )
        ...
        # exp(-j*2*pi*k*n/N), including the Hann window.
        self.kernel = (
            self.window
            * np.exp(
                -2j
        ...
        # One-sided density normalization for a non-DC,
        # non-Nyquist bin:
        #
        # PSD = 2*|DFT|^2 / (fs*sum(window^2))
        self.psd_scale = (
            2.0
            / (
                sample_rate_hz
    ...
    def analyze(
        ...
        Analyze a matrix shaped:
            ...
            number_of_windows x samples_per_window
        ...
        if windows.ndim != 2:
            raise ValueError(
                "Windows must be a two-dimensional matrix."
            )
        ...
        if (
            ...
            raise ValueError(
                "Unexpected samples per subwindow."
            )
        ...
        # Matrix-vector multiplication calculates the selected DFT
        # coefficient for every qualifying window at once.
...
def prepare_window_matrix(
    ...
    if samples_per_window < 4:
        raise RuntimeError(
            "Subwindow contains too few samples."
        )
...
def cluster_summaries(
    ...
    sums = np.fromiter(
        (
            float(np.sum(cluster))
            for cluster in acquisition_clusters
        ),
        dtype=np.float64,
...
def pooled_mean_watts(
    ...
    sums, counts = cluster_summaries(
        acquisition_clusters
    )

    return pooled_mean_from_summaries(
        sums,
        counts,
    )
...
def cluster_bootstrap_ci(
    ...
    if number_of_clusters == 1:
        value = pooled_mean_from_summaries(
            cluster_sums,
            cluster_counts,
        )
        return value, value
    ...
    while completed < repetitions:
        ...
        sampled_sums = np.sum(
            cluster_sums[selected_indices],
            axis=1,
        )

        sampled_counts = np.sum(
            cluster_counts[selected_indices],
            axis=1,
        )
...
def compare_recent_blocks(
    ...
    if (
        len(acquisition_clusters)
        < required_clusters
    ):
        return math.nan, math.nan, math.nan

    preceding_clusters = acquisition_clusters[
        -required_clusters:-block_size
    ]

    recent_clusters = acquisition_clusters[
        -block_size:
    ]

    preceding_mean = pooled_mean_watts(
        preceding_clusters
    )

    recent_mean = pooled_mean_watts(
        recent_clusters
    )

    if (
        not math.isfinite(preceding_mean)
        or preceding_mean <= 0
    ):
        return (
            preceding_mean,
    ...
    relative_change = (
        abs(recent_mean - preceding_mean)
        / preceding_mean
    )
...
def check_convergence(
    ...
    current_mean_watts = pooled_mean_watts(
        acquisition_clusters
    )

    ci_low_watts, ci_high_watts = (
    ...
    relative_ci_half_width = (
        (ci_high_watts - ci_low_watts)
        / (2.0 * current_mean_watts)
    ...
    (
        ...
        recent_mean_watts,
        block_relative_change,
    ) = compare_recent_blocks(
        acquisition_clusters,
        STABILITY_BLOCK_ACQUISITIONS,
    )
...
def main() -> None:
    ...
    run_directory = get_next_run_directory(
        base_directory
    )
    ...
    try:
        ...
        print(
            ...
            "vectorized single-bin Hann DFT"
        )
        print(
            "Mean calculation       : "
            "running sum in watts"
        )
        print(
            f"Precision target       : "
        ...
        while (
            not stop_requested
            and not converged
            and long_acquisition_number
            < MAX_LONG_ACQUISITIONS
        ):
            acquisition_started = (
                time.perf_counter()
            )
            ...
            try:
                ...
                # Freeze a completed acquisition.
                scope.query(":SINGle;*OPC?")

                waveform = (
                    read_channel_waveform_binary(
                        scope
                    )
                )

                voltage_volts = waveform[
                    "voltage_volts"
                ]
                ...
                sample_interval_s = waveform[
                    "sample_interval_s"
                ]

                time_origin_s = waveform[
                    "time_origin_s"
                ]
                ...
                if (
                    analyzer is None
                    or samples_per_window
                    != analyzer_samples_per_window
                    or not math.isclose(
                        sample_rate_hz,
                ...
                current_mean_dbm = watts_to_dbm(
                    cumulative_power_watts
                    / qualifying_pair_count
                ) if qualifying_pair_count else math.nan
        ...
        if not acquisition_clusters:
            print(
                "No qualifying measurements were collected."
            )
            return
        ...
        final_ci_low_dbm = watts_to_dbm(
            final_ci_low_watts
        )

        final_ci_high_dbm = watts_to_dbm(
            final_ci_high_watts
        )
        ...
        ) if all_powers.size > 1 else math.nan
        ...
        print(
            f"Long acquisitions tried : "
            f"{long_acquisition_number}"
        )
        print(
            f"Qualifying acquisitions : "
            f"{qualifying_acquisition_count}"
        )
        print(
            f"Qualifying pairs        : "
        ...
        print(
            ...
            f"{final_ci_low_dbm:.6f} to "
            f"{final_ci_high_dbm:.6f} dBm"
        )
        print(
            f"Relative CI half-width  : "
            f"{relative_ci_half_width * 100:.3f}%"
        )
        ...
        result = {
            ...
            "final_mean_dbm":
                final_mean_dbm,
            "final_ci95_low_watts":
                final_ci_low_watts,
            "final_ci95_high_watts":
                final_ci_high_watts,
            "final_ci95_low_dbm":
                final_ci_low_dbm,
            "final_ci95_high_dbm":
